...
describe("NTP timestamps", function () {
  ...
  describe("Conversion", function () {
    ...
    it("should return Jan 01 1901 GMT for one year in seconds", function () {
      const oneYear = (new Date("Jan 01 1901 GMT").getTime() - new Date("Jan 01 1900 GMT").getTime()) / 1000;
      let oneYearAsBits = (oneYear >>> 0).toString(2);
      oneYearAsBits = "0".repeat(32).substr(0, 32 - oneYearAsBits.length) + oneYearAsBits;

      assert.equal(
        NtpPacketParser._fromNtpTimestamp(oneYearAsBits + "0".repeat(32)).getTime(),
